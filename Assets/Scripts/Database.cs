...
public partial class Database : MonoBehaviour
{
    ...
    class characters
    {
        ...
        public int forceStat { get; set; }
        public int healthStat { get; set; }
        public int defenseStat { get; set; }
        public int spiritStat { get; set; }
        public int soulStat { get; set; }
        public int chakraStat { get; set; }
    ...
    void LoadSkills(PlayerSkills skills)
    {
        ...
        foreach (character_skills row in connection.Query<character_skills>("SELECT * FROM character_skills WHERE character=?", skills.name))
        {
            ...
            if (index != -1)
            {
                ...
                // them back again.
                skill.castTimeEnd = NetworkTime.time - 1f;
    ...
    public GameObject CharacterLoad(string characterName, List<Player> prefabs, bool isPreview)
    {
        ...
        if (row != null)
        {
            ...
            if (prefab != null)
            {
                ...
                player.powerStat.value = row.powerStat;
                player.forceStat.value = row.forceStat;
                player.healthStat.value = row.healthStat;
                player.defenseStat.value = row.defenseStat;
                player.spiritStat.value = row.spiritStat;
                player.soulStat.value = row.soulStat;
                player.chakraStat.value = row.chakraStat;
                ...
                player.chakra.current = row.chakra;
    ...
    public void CharacterSave(Player player, bool online, bool useTransaction = true)
    {
        ...
        connection.InsertOrReplace(new characters{
            ...
            chakra = player.chakra.current,
            powerStat = player.powerStat.value,
            forceStat = player.forceStat.value,
            healthStat = player.healthStat.value,
            defenseStat = player.defenseStat.value,
            spiritStat = player.spiritStat.value,
            soulStat = player.soulStat.value,
            chakraStat = player.chakraStat.value,
