...
public partial struct Buff
{
    ...
    [HideInInspector] public Vector2 substitutePoint;

    // constructors
    public Buff(BuffSkill data, int level, Vector2 wantedSubstitutePoint)
    {
        hash = data.name.GetStableHashCode();
        this.level = level;
        buffTimeEnd = NetworkTime.time + data.buffTime.Get(level); 
        substitutePoint = wantedSubstitutePoint;
    }
    ...
    public SubstituteSkillEffect substituteEffect => data.substituteEffect;
