...
[RequireComponent(typeof(Chakra))]
...
{
    public enum Villages { None, Leaf, Sand, Mist, Cloud, Stone, Rain, Sound }

    [Header("Components")]
    public Level level;
    public Health health;
    public Chakra chakra;
    public FlickerEnergy flickerEnergy;
...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    public Villages village = Villages.Leaf;
    public enum States { Idle, Moving, Casting, Stunned, Dead, ChargingChakra, Blocking };
    ...
    [HideInInspector] public double castTimeEnd;
    ...
    void Update()
    {
        ...
        {
            lookDirection = (velocity * 10f).normalized;
            //lookDirection = Utils.OrthonormalVector2(velocity, lookDirection);
        }
        //facetarget? else if (target != null) lookDirection = Utils.OrthonormalVector2(target.transform.position - transform.position, lookDirection);
    ...
    public virtual bool CanAttack(Entity entity)
    {
        return health.current > 0 &&
               ...
               entity != this &&
               !inSafeZone && !entity.inSafeZone;// && !NavMesh2D.Raycast(transform.position, entity.transform.position, out NavMeshHit2D hit, NavMesh2D.AllAreas);;
    }
    ...
    [ClientRpc]
    public void RpcSetLookDirection(Vector2 wantedLookDirection)
    {
        lookDirection = wantedLookDirection;
    }
}
