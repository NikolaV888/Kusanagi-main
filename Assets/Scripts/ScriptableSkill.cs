...
public abstract partial class ScriptableSkill : ScriptableObject
{
    ...
    public bool canBeHotbarred;
    public bool increaseAnimationIndex = false;
    ...
    public Vector2 substitutePoint;
    public LinearInt chakraCosts;
    public LinearInt flickerPointCosts;
    public LinearFloat castTime;
    public LinearFloat selfStunTime;
    public LinearFloat cooldown;
    public LinearFloat castRange;
    public ScriptableSkill followUpSkill;
    public bool isFollowup = false;
    public GameObject casterVFX;
    public GameObject targetVFX;
    public bool ignoreCastAnimation = false;
    ...
    {
        if(!caster.flickerEnergy) return caster.health.current > 0 &&
               caster.chakra.current >= chakraCosts.Get(skillLevel) &&
               CheckWeapon(caster);
        ...
        return caster.health.current > 0 &&
               caster.chakra.current >= chakraCosts.Get(skillLevel) &&
               caster.flickerEnergy.current >= flickerPointCosts.Get(skillLevel) &&
               CheckWeapon(caster);
    }
    ...
    {
        if (increaseAnimationIndex) caster.combat.animationIndex++;;
    ...
    {
        if (caster.audioSource != null && castEndSound != null)
            caster.audioSource.PlayOneShot(castEndSound, 1f);
    ...
    */
    public virtual string ToolTip(int level, bool showRequirements = false)
    {
        ...
        tip.Replace("{CHAKRACOST}", chakraCosts.Get(level).ToString());
