...
public partial class Player : Entity
{
    ...
    public ForceStat forceStat;
    public HealthStat healthStat;
    public DefenseStat defenseStat;
    public SpiritStat spiritStat;
    public SoulStat soulStat;
    public ChakraStat chakraStat;
    ...
    public enum Ranks { Academy_Student, Genin, Chunin, Special_Jonin, Jonin, Kage }
    public Ranks rank = Ranks.Academy_Student;
    public bool isRogue = false;
    [SyncVar, HideInInspector] public bool isPremium = false;
    [SyncVar, HideInInspector] public bool ignoreCastAnimation = false;
    ...
    public bool actuallyCasting = false;
    void LateUpdate()
    {
        ...
        // => skill names are assumed to be boolean parameters in animators
        ...
        if (isClient) // no need for animations on the server
        {
            ...
            {
                ignoreCastAnimation = skills.skills[skills.currentSkill].ignoreCastAnimation;
                Debug.Log("waitingOnFollowupJutsu 1: " + skills.currentSkill);
                if (skills.skills[skills.currentSkill].followUpSkill)
                {
                    waitingOnFollowupJutsu = true;
                    Debug.Log("waitingOnFollowupJutsu 2: " + waitingOnFollowupJutsu);
                }
                else waitingOnFollowupJutsu = false;
            }*/
            ...
            animator.SetFloat("LookX", Utils.OrthonormalVector2(lookDirection, lookDirection).x);
            animator.SetFloat("LookY", Utils.OrthonormalVector2(lookDirection, lookDirection).y);
        }
    }
    ...
    [Command]
    public void SetChargingChakra(bool value)
    {
        chargingChakra = value;
    }
    ...
    string UpdateServer_CASTING()
    {
        ...
        if (EventSkillFinished())
        {
            ...
            if (skill.followUpSkill)
            {
                ...
                if (skillID > 0)
                {
                    Debug.Log("Using Follow Up Skill");
                    skills.currentSkill = skillID;
                    skills.StartCast(skills.skills[skillID]);
                }
                else skills.currentSkill = -1;
            }
            else skills.currentSkill = -1;
    ...
    {
        health.recover = remainingLogoutTime < 0.1f;
        ...
        if (state == States.Blocking.ToString()) return UpdateServer_BLOCKING();
    ...
    protected override void UpdateClient()
    {
                ...
                if (useSkillWhenCloser != -1)
                {
                    ...
                    if (CanAttack(target))
                    {
                        ...
                        if (Utils.ClosestDistance(collider, target.collider) <= range)
                        {
                            ...
                            ((PlayerSkills)skills).CmdUse(useSkillWhenCloser, lookDirection, false);
        ...
        else if (state == States.ChargingChakra.ToString()) { }
        else if (state == States.Blocking.ToString()) { }
    ...
    protected override void UpdateOverlays()
    {
        ...
        if (nameOverlay != null)
        {
            ...
            if (localPlayer != null)
            {
                if (localPlayer.party.InParty() && localPlayer.party.party.Contains(name))
    ...
    public void OnSkillCastFinished(Skill skill)
    {
        ...
        if(skill.followUpSkill && false)
        {
            ...
            {
                Debug.Log("Using Follow Up Skill");
                ((PlayerSkills)skills).TryUse(skillID, true);
            }
            else Debug.Log("Player does not know followup jutsu..");
        }
        else if (false && pendingSkill != -1)
        {
            ((PlayerSkills)skills).TryUse(pendingSkill, true);
        }
        // otherwise do follow up attack if no interruptions happened
        else if (skill.followupDefaultAttack && false)
        {
    ...
    public void OnDamageDealtTo(Entity victim)
    {
        ...
        if (victim is Player && ((Player)victim).IsInnocent() && victim.village == village)
        {
            // start offender if not a murderer yet
            if (!IsMurderer()) StartOffender();
        }
        // attacked a pet with an innocent owner
        else if (victim is Pet && ((Pet)victim).owner.IsInnocent() && victim.village == village)
        {
            // start offender if not a murderer yet
            if (!IsMurderer()) StartOffender();
        }
