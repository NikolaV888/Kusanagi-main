...
public class PlayerSkills : Skills
{
    ...
    [Command]
    public void CmdUse(int skillIndex, Vector2 direction, bool ignoreStun)
    {
        if(ignoreStun)
        {
            entity.stunTimeEnd = NetworkTime.time - 1;
        }

        // validate
        if ((entity.state == "IDLE" || entity.state == "MOVING" || entity.state == "CASTING" || ignoreStun) &&
            0 <= skillIndex && skillIndex < skills.Count)
        {
            Debug.Log("CMD USE 2:" + skillIndex);
            // skill learned and can be casted?
            if (skills[skillIndex].level > 0 && (skills[skillIndex].IsReady() || !skills[skillIndex].IsOnCooldown()))//temporary change, may cause issues
            {
            ...
            else if (skills[skillIndex].level <= 0) Debug.Log("Error CMDUse, skill not learned. '" + skills[skillIndex].name + "'");
            else Debug.Log("Error CMDUse, skill not ready. '" + skills[skillIndex].name + "'");
        }
    }

    [Client]
    public void TryUse(int skillIndex, bool ignoreState = false, bool ignoreStun = false)
    {
        // only if not casting already
        // (might need to ignore that when coming from pending skill where
        //  CASTING is still true)
        Debug.Log("TryUse " + skillIndex + ", ignoring?" + ignoreState);
        if (entity.state != "CASTING" || ignoreState)
        {
            ...
            if (CastCheckSelf(skill) && CastCheckTarget(skill))
            {
                ...
                {
                    Debug.Log("CMD Use jutsu " + skill.name);
                    // cast
                    CmdUse(skillIndex, ((Player)entity).lookDirection, ignoreStun);
                }
                else Debug.Log("Jutsu Fail 2");
            }
            else Debug.Log("Jutsu Fail 1");
        }
        else
        {
            ((Player)entity).pendingSkill = skillIndex;
            Debug.Log("Jutsu Fail 3");
        }
    }
