...
public abstract class Skills : NetworkBehaviour, IHealthBonus, IChakraBonus, ICombatBonus
{
    [Header("Components")]
...
#pragma warning restore CS0649 // Field is never assigned to
    ...
    public int GetChakraBonus(int baseChakra)
    {
        // sum up manually. Linq.Sum() is HEAVY(!) on GC and performance (190 KB/call!)
        int passiveBonus = 0;
        foreach (Skill skill in skills)
    ...
    public int GetChakraRecoveryBonus()
    {
        // sum up manually. Linq.Sum() is HEAVY(!) on GC and performance (190 KB/call!)
        float passivePercent = 0;
        foreach (Skill skill in skills)
        ...
        return Convert.ToInt32(passivePercent * chakra.max) + Convert.ToInt32(buffPercent * chakra.max);
    }
    ...
    {
        Debug.Log("Start Casting " + skill.name);
    ...
    public void FinishCast(Skill skill)
    {
        ...
        if (CastCheckSelf(skill, false) && CastCheckTarget(skill))
        {
            ...
            try
            {
                Debug.Log("Skill Applied");
                skill.Apply(entity, currentSkillDirection);
            }
            catch(Exception e)
            {
                Debug.Log("Need to debug: " + e.Message);
            }
            // rpc for client sided effects
            ...
            chakra.current -= skill.chakraCost;
            if(flickerEnergy) flickerEnergy.current -= skill.flickerCost;
